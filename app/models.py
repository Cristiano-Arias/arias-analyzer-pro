...
class TR(db.Model):
    ...
    #
    # A estrutura original previa um único campo ``credenciamento_observacoes`` que
    # combinava requisitos de credenciamento e observações finais. Para dar
    # suporte às mudanças requisitadas no fluxo de TR – separar o conteúdo de
    # credenciamento e observações – foram adicionados dois novos campos
    # independentes: ``credenciamento`` e ``observacoes``.  Ambos são
    # opcionais e armazenados como texto livre.
    credenciamento = db.Column(db.Text)
    observacoes = db.Column(db.Text)

    #
    # Valores de orçamento e prazo máximo de execução são mantidos no TR para
    # que o comprador possa visualizá‑los quando receber o Termo de
    # Referência.  Embora não façam parte do escopo essencial do TR, estes
    # campos precisam estar presentes e podem ser preenchidos pelo
    # requisitante.
